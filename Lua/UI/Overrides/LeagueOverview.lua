...
function Update( bForceViewModel )
	...
	if(pLeague ~= nil) then
		...
		-- Vox Deorum: Populate last session results if no active proposals
		local sessionResults = VD_GetSessionResults()
		if (not pLeague:IsInSession() and #sessionResults > 0 and #model.Proposals == 0) then
			model.LastTurnResults = {};
			for _, result in ipairs(sessionResults) do
				local choice = result.Choice or kChoiceNone;
				table.insert(model.LastTurnResults, {
					Text = pLeague:GetResolutionName(result.Type, -1, choice, false),
					ToolTip = pLeague:GetResolutionDetails(result.Type, iActivePlayer, -1, choice),
					Direction = result.IsEnact and "Enact" or "Repeal",
					Passed = result.Passed,
					ProposalPlayer = result.ProposalPlayer,
				});
			end
		end
...
function ViewCurrentProposals(model)
	...
	-- Vox Deorum: Show last session voting results
	if (model.LastTurnResults and #model.LastTurnResults > 0) then
		-- Header
		local headerInstance = m_ProposalManager:GetInstance();
		headerInstance.ResolutionButton:SetDisabled(true);
		headerInstance.ResolutionButton:ClearCallback(Mouse.eLClick);
		headerInstance.ResolutionName:SetText("[COLOR_YELLOW]Last Session Results[ENDCOLOR]");
		headerInstance.ResolutionButton:SetToolTipString("");
		headerInstance.VoteBox:SetHide(true);
		headerInstance.AltIconFrame:SetHide(true);
		headerInstance.SubIconFrame:SetHide(true);
		headerInstance.VoteStatus:SetHide(true);
		IconHookup(0, 64, "INTL_PROJECT_ATLAS_EXP2", headerInstance.Icon);
		local headerW, _ = headerInstance.ResolutionButton:GetSizeVal();
		headerInstance.ResolutionButton:SetSizeVal(headerW, 40);

		-- Result entries
		for _, result in ipairs(model.LastTurnResults) do
			local instance = m_ProposalManager:GetInstance();
			instance.ResolutionButton:ClearCallback(Mouse.eLClick);
			instance.ResolutionButton:SetDisabled(true);

			local prefix = result.Direction == "Enact" and g_strEnactPrefix or g_strRepealPrefix;
			local statusText;
			if result.Passed then
				statusText = "[COLOR_GREEN]PASSED:[ENDCOLOR] ";
			else
				statusText = "[COLOR_RED]FAILED:[ENDCOLOR] ";
			end

			instance.ResolutionName:SetText(statusText .. prefix .. result.Text);
			instance.ResolutionButton:SetToolTipString(result.ToolTip);

			if (result.ProposalPlayer ~= nil and result.ProposalPlayer ~= -1 and Players[result.ProposalPlayer] ~= nil) then
				local player = Players[result.ProposalPlayer];
				...
				CivIconHookup(result.ProposalPlayer, 32, instance.SubIcon, instance.SubIconBG, instance.SubIconShadow, true, true);
				instance.SubIconFrame:SetHide(false);
			else
				IconHookup(0, 64, "INTL_PROJECT_ATLAS_EXP2", instance.Icon);
				instance.SubIconFrame:SetHide(true);
			...
			local w, _ = instance.ResolutionButton:GetSizeVal();
			local _, h = instance.ResolutionName:GetSizeVal();
			instance.ResolutionButton:SetSizeVal(w, math.max(64, h + 30));
		end
	end
