...
--local isCityDescsOn = Game.IsOption("GAMEOPTION_JFD_AIOBSERVER_ENABLE_CITYDESCS");
local isCityDescsOn = false
...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	--JFD
	if isCityDescsOn then
		controls.CityDescriptor:SetColor(textColor, 0);
		controls.CityDescriptor:SetColor(textColorShadow, 1);
		controls.CityDescriptor:SetColor(textColorSoft, 2);
		controls.CityName2:SetColor(textColor, 0);
		controls.CityName2:SetColor(textColorShadow, 1);
		controls.CityName2:SetColor(textColorSoft, 2);
	end	
	...
	if city ~= nil then
		...
		--JFD
		if isCityDescsOn then
			local cityDescriptor = player:GetCityDescriptor(city)
			local localizedCityDescriptor = Locale.ConvertTextKey(cityDescriptor);
			controls.CityDescriptor:SetText(localizedCityDescriptor);
			controls.CityName2:SetText(convertedKey);
			controls.CityName:SetHide(true);
			controls.NameLabelStack:SetHide(false);
		end
		--JFD	
